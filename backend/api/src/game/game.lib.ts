...
export class GameRoom {
  ...
  ballMove() {
    ...
    this.gameObject.ball.left = Math.min(
      ballRightMax, Math.max(0, this.gameObject.ball.left + this.ballDirection.moveX)
    );
    this.gameObject.ball.top = Math.min(
      ballBottomMax, Math.max(0, this.gameObject.ball.top + this.ballDirection.moveY)
    );
    ...
    if (this.gameObject.bar1.top <= this.gameObject.ball.top + ballBaseSize &&
        this.gameObject.ball.top <= this.gameObject.bar1.top + barBaseHeight &&
        this.gameObject.ball.left <= this.gameObject.bar1.left + barBaseWidth &&
        this.ballDirection.moveX < 0) {
      this.setBallBounce(this.gameObject.bar1.top);
    }
    // Reflection from hitting the bar on the right
    if (this.gameObject.bar2.top <= this.gameObject.ball.top + ballBaseSize &&
        this.gameObject.ball.top <= this.gameObject.bar2.top + barBaseHeight &&
        this.gameObject.bar2.left <= this.gameObject.ball.left + ballBaseSize &&
        this.ballDirection.moveX > 0) {
  ...
  play() {
    ...
    this.interval = setInterval(() => {
      this.ballMove();
      if (this.gameObject.ball.left == 0 || this.gameObject.ball.left == (widthBaseSize - ballBaseSize)) {
        this.turnFinish();
      }
      else {
